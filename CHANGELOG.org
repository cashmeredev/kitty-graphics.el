...
When the same org buffer is shown in several windows, one canonical window per buffer and terminal (preferring the selected window) renders the scaled headings; the other windows show plain heading text instead of flickering. Per-window heading rendering is a known limitation.
